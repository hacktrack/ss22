...
var wysiwygStylesheet = "skin/fresh/css/wysiwyg.content.css";
...
function wysiwygInit()
{
	...
	if (1==1 || typeof(document.contentEditable) == "string" || (typeof(document.designMode) == "string" && (document.all || document.designMode == "off")))
	{
		...
		for (var i = 0; i < theTextareas.length; i++)
		{
			...
			if (theTextarea.className.classExists("wysiwyg") && theTextarea.getAttribute('data-format') == 'text/html')
			{
				if (theTextarea.id == "")
				{
				...
				if (!window.hasOwnProperty('wysiwygInstances')) {
					window.wysiwygInstances = {};
				}

				setTimeout("window.wysiwygInstances['" + theTextarea.id + "'] = new wysiwygEditor('" + theTextarea.id + "')", 500 * (i));
			}
		}
	}
	else
...
{
	var iHTML=document.getElementById(replacedTextareaID + '_html').innerHTML;
	...
	this.theCleaner = document.createElement("div");
	...
	// Input for result html
	this.resultInput.type = "hidden";
	this.resultInput.id = this.theTextarea.id + "wysiwygResult";
	this.resultInput.name = this.theTextarea.getAttribute('data-result-name');
	...
	this.theCleaner.setAttribute('style','clear:both');
	...
	this.theIframe.setAttribute('src','skin/fresh/tpl/blank.tpl');
	...
	this.theContainer.appendChild(this.theCleaner);
	this.theContainer.appendChild(this.theInput);
	this.theContainer.appendChild(this.theExtraInput);
	this.theContainer.appendChild(this.resultInput);
	...
	this.writeDocument(DOMPurify.sanitize(this.theInput.value));
...
wysiwygEditor.prototype.initEdit = function()
{
	...
	remapKeys(this.theIframe.contentWindow.document);
...
wysiwygEditor.prototype.modifyFormSubmit = function()
{
	...
	theForm.onsubmit = function()
	{
		...
		self.resultInput.value = self.theInput.value;
	
		
		if(window.wysiwygOn)
		{
			// clean up
			var ready=self.resultInput.value;
...
wysiwygEditor.prototype.switchMode = function()
{
	if (!this.locked)
	{
		...
		if (this.wysiwyg)
		{
			...
			// Remove all html tags from wysiwyg text
            this.theTextarea.value = this._stripTags(this.theInput.value);
...
wysiwygEditor.prototype._stripTags = function(html)
{
	var elm = document.createElement('div'),
		elms;

	// remove new lines between table cell tags and replace them with tabs to keep tables visually more consistent with html mode
	html = html.replace(/\<\/td\s*\>\s*\<td/gi,'</td>\t<td');
	...
	var removeTags = ['style', 'script'];
	for (var i=0; i<removeTags.length; i++){
		elms = elm.getElementsByTagName(removeTags[i]);
			...
			else
			{
				if (theIframe.contentWindow.document.selection)
				{
					...
					if (theSelection == "")
					{
						...
						alert(document.getElementById('lang-mail_compose-select_text').value);
						break;
					}
				}
				else
				...
				var theURL = prompt(document.getElementById('lang-mail_compose-enter_url').value, "http://");
