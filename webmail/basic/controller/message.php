...
class slControllerMessage extends slControllerDefault
{
	...
	{	
		...
		try{
			...
			$oPrimaryAccount = User::getCurrentUser()->getAccount($_SESSION['EMAIL']);
			...
			$result->redirectURL = $request->getPath().'?_n[p][content]=mail.compose';
			try{
	 	...
	 	public function save()
	{
		...
		try{
			...
			$link = str_replace('&amp;','&',$router->getLink('main','content','mail.compose'));
			$link .= '&_n[p][main]=win.main.tree&_s[item]='.$item.'&_s[id]='.$folder.'&_s[type]=M&_s[action]=continue';
			if($request->get('all._frm.sourceid')){
				...
				$action = $request->get('all._frm.mail_action')=='drafts'?$request->get('all._frm.source_action'):$request->get('all._frm.mail_action');
				$link.='&_s[sourceaction]='.$request->get('all._frm.sourceaction');
			}
			$result->redirect = true;
	 	...
	 	public function cancel()
	{
		$result = new stdclass();
		$result->redirect = true;
		$result->redirectURL = 'index.html?_n[w]=main&_n[p][main]=list.mail';
		return $result;
	}
	...
	public function cancel_select()
	{
		$request = slRequest::instance();
		$session = slSession::instance();
		$dialog = str_replace('.','_',$this->application->pages['content']);
		$this->loadSentData($dialog);
	...
	private function loadComposeFromSession($id)
	{
		$result = new stdClass();
		$result->redirect = true;
		$result->redirectURL = 'index.html?_n[p][content]=mail.compose&_n[p][main]=win.main.tree&_n[w]=main&_s[action]=session&_s[message_id]='.$id;
		return $result;
	...
	private function appendAdressesFromDialog($contacts,&$variable)
	{
		...
		foreach($selected as $cnt){
			$contacts[$cnt]['address'] = $cnt;
			$contacts[$cnt]['selected'] = 'on';
		}
		if($contacts) foreach($contacts as $id => $contact){
			if($contact['selected']){
				$contact = MailParse::parseAddresses($contact['address']);
				@$contact = reset($contact);
				$var[0] = $contact['address'];
				if($contact['display'] && $contact['display']!=$contact['address']){
					$var[1] = strtr($contact['display'],array("\""=>"\\\"","\\"=>"\\\\"));;
				}
				$variable[] = $var;
			}
		}
	...
	{	
		if($items) foreach($items as $id => $item){
			if($item['selected']){
				$class = $item['class'];
	...
	private function createMessage($send = false,&$failedAttachments = array(),$isDraft = false)
	{
		...
		if($this->info['rfc_time']){
			$mail->RFCDate = $this->info['rfc_time'];
		}
	...
	static public function addAttachment(&$oMail, $class, $type, $data, $cid = false)
	{
		...
		switch ($class){
			...
			case 'item':
				...
				$file = $oItem->getItemData($info);
				switch ($type)
				{
					case 'normal':
				
						$oMail->AddStringAttachment($file, $description?$description:$info['name'], $info['encoding'], $info['mimetype'],
						$info['encoding'] == 'base64' ? true : false);
						break;
					default:
						throw new Exc('message_attachment_type', $type);
				}
