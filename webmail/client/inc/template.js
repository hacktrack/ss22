 ...
 */
_me.tmp = function(sName,aData){
	return this.exe(storage.template(sName),aData);
};
...
_me.parser = function(){
	...
	for (;;) {
		...
		// Note: this is without quote escaping
		case 'htmlspecialchars':
			this.sBuffer += (this.variable(elm11,this.strict)).toString().escapeHTML();
			break;
		
		// when htmlspecialchars is not enough and you need to escape also double quotes
		// example of usage: <p title="{htmlspecialchars-double-quote title}">
		case 'htmlspecialchars-double-quote':
			this.sBuffer += (this.variable(elm11,this.strict)).toString().escapeHTML().replace(/"/g, '&quot;');
			break;
...
_me.include = function(elm11){
	...
	if (elm11){
		...
		try{
			...
			console.log('template.include', 'JSON error');
		}
...
_me.loop = function(elm11,bInvert){
	...
	var exp1 = new RegExp('{' + elm11 + '::\\*', 'g'),
		exp2 = new RegExp('[ ]' + elm11 + '::\\*', 'g'),
		exp3 = new RegExp('::' + elm11 + '::\\*', 'g');
