...
require_once('IceWarpExternalLogin.class.php');

 
function currentPageURL() {
	$pageURL = 'http';
	if ($_SERVER["HTTPS"] == "on" || ($_SESSION['ALWAYSHTTPS'] ?? false)) {$pageURL .= "s";}
	$pageURL .= "://";
...
$error_description=array(
	...
	'no_login_sid'		 => 'SID could not be retrieved from IW server.'
);
...
if(isset($_POST['retrieve_sid']) ||isset($_POST['retrieve_url']) || isset($_POST['login_goto']))
{
	...
	$oRemote = new IceWarpExternalLogin(array(
...
<form action="" method="post">
	<table>
		...
		<tr>
			<td>Interface: </td>
			<td>
				<select size="1" name="login[interface]">
					<option value="advanced">Advaced</option>
