...
{
	meeting.settings.isRunning=true;
...
meeting.status.serverMute=function(muted)
{
	if(muted)
		$('#muteButton').addClass('forced');
	else
		$('#muteButton').removeClass('forced');

	$('#muteButton').attr("disabled", muted);
}
...
meeting.status.getString=function(code)
{
	...
	return text;
//	return document.getElementById(code) && document.getElementById(code).value || '';
}
