 ...
 * Extended version for IceWarp ltd. by Martin Ekblom 2014-2016
 */
...
var JsSIP = (function() {
  ...
  Object.defineProperties(JsSIP, {
    version: {
      get: function(){ return '0.4.0-icewarp'; }
    },
    name: {
      get: function(){ return 'IceWarp WebClient with JsSIP'; }
    }
  });
...
var RTCMediaHandler = function(session, constraints) {
  ...
  this.localMedia = null; // Stream from local camera
  this.streams = [];      // Streams from remote party
...
RTCMediaHandler.prototype = {
  ...
  addStream: function(stream, onSuccess, onFailure, constraints) {
    if(this.peerConnection.signalingState!='closed') try {
      // Plugin requires demands only one argument
  ...
  */
  init: function(options) {
    ...
    // Streams added by remote peer
    this.peerConnection.onaddstream = function(e) {
      // Keep track of added remote streams
      self.streams.push(e.stream);
      self.logger.log('stream added: '+ e.stream.id);

      // Notify about additional streams
      if(self.onAddStream)
        self.onAddStream();
    ...
    this.peerConnection.onremovestream = function(e) {
      ...
      // @todo: add logic for removing streams from stream array
    };
    ...
    this.peerConnection.ongatheringchange = function(e) {
      self.logger.log('ICE gathering state changed to "'+ this.iceGatheringState +'"');
    ...
    this.peerConnection.oniceconnectionstatechange = function() {
      ...
      if(this.iceConnectionState=='failed') {
        self.session.emit('failed', self.session, {
          originator: 'system',
          cause: JsSIP.C.causes.ICE_FAILED
        });
...
/*    this.peerConnection.onnegotiationneeded = function() {
      if(self.session.dialog && self.session.isReadyToReinvite()) {
        self.logger.log('Renegotiation needed, sending reINVITE');
        self.session.sendReinvite();
      }
    }
*/
  ...
  close: function() {
    ...
    if(this.peerConnection) {
      ...
      // Stopping local camera and microphone, with workaround for plugin
      if(navigator.browser && navigator.browser.application=='MSIE') {
        // In IE with plugin just stop
        ...
        while(idx--) {
          if(streams[idx].stop)
            ...
            var tracks = streams[idx].getTracks();
            var n = tracks.length;
            while(n--)
              tracks[n].stop();
          }
  ...
  */
  getUserMedia: function(onSuccess, onFailure, constraints) {
    ...
    JsSIP.WebRTC.getUserMedia(constraints,
      success,
      function(e) {
        // Chrome reports total failure if only webcam is missing, copy FF behaviour
        if('chrome' in window && constraints.video && e.name=='DevicesNotFoundError') {
...
INFO = function(session) {
  ...
  this.logger = session.ua.getLogger('jssip.rtcsession.info', session.id);
  this.owner = session;
  this.direction = null;

  this.initEvents(events);
};
INFO.prototype = new JsSIP.EventEmitter();
 ...
 * @private
 */
INFO.prototype.onDialogError = function(response) {
  this.emit('failed', this, {
    originator: 'remote',
    response: response,
    cause: JsSIP.C.causes.DIALOG_ERROR
...
INFO.prototype.init_incoming = function(request) {
  ...
  this.owner.emit('newInfo', this.owner, {
    originator: 'remote',
    info: this,
    request: request
  });
...
// @TODO: DTMF should inherit from INFO and only override constructor, send and init_incoming methods
    ...
    answerCreationSucceeded = function(body) {
      var
        ...
        replySucceeded = function() {
          ...
          // Chrome has audio and video streams ready already here
          if(self.rtcMediaHandler.streams.length && (self.rtcMediaHandler.streams[0].getAudioTracks().length || self.rtcMediaHandler.streams[0].getVideoTracks().length))
            self.started('local');
          else
            // Firefox requires waiting until the stream event fired
            self.rtcMediaHandler.onAddStream = function() {
              self.started('local');
              self.rtcMediaHandler.onAddStream = undefined;
            }
...
RTCSession.prototype.connect = function(target, options) {
  ...
  if (options.guest) {
    requestParams.from_display_name = options.guest.name;
    requestParams.from_uri = 'sip:' + options.guest.email;
  }
      ...
      case JsSIP.C.INFO:
        if(this.status === C.STATUS_CONFIRMED || this.status === C.STATUS_WAITING_FOR_ACK) {
          ...
          if (contentType && (contentType.match(/^application\/dtmf-relay/i))) {
            ...
            new INFO(this).init_incoming(request);
          }
        }
    }
  }
...
RTCSession.prototype.sendInitialRequest = function(mediaConstraints, RTCOfferConstraints, mediaStream) {
 ...
 if(mediaConstraints.screen && !('chrome' in window))
  delete mediaConstraints.screen;
 ...
 if (mediaStream) {
   ...
   userMediaSucceeded(mediaStream);
    ...
    // Try to share screen (will require Chrome with plugin and https)

  JsSIP.WebRTC.getScreenId(function(error, screen_capture_id) {
...
RTCSession.prototype.receiveInviteResponse = function(response) {
  ...
  switch(true) {
    ...
    case /^2[0-9]{2}$/.test(response.status_code):
      ...
      // Parse sdp to extract media stream descriptions
      ...
      var direction = {};
      while(idx--) {
        // Check for inactive video stream from remote 
        if(sdp.media[idx].type==='video' && sdp.media[idx].direction==='inactive') {
          removeVideo = true;
        }
        // Check for expected streams and direction
        direction[sdp.media[idx].type] = sdp.media[idx].direction || 'sendrecv';
      }

      // Remove local media if denied by remote party
      if(this.rtcMediaHandler.localMedia) {
      ...
      this.rtcMediaHandler.onMessage(
        ...
        function() {
          ...
          // Chrome has audio and video streams ready already here (or if we are not expecting any incoming stream, we can start already here)
          if(
            direction.audio==='recvonly' && !direction.video || // Only sending audio, no incoming streams expected
            !direction.audio && direction.video==='recvonly' || // Only sending video, no incoming streams expected
            direction.audio==='recvonly' && direction.video==='recvonly' || // Only sending audio and video, no incoming stream expected
            session.rtcMediaHandler.streams.length && (session.rtcMediaHandler.streams[0].getAudioTracks().length || session.rtcMediaHandler.streams[0].getVideoTracks().length) // Expect incoming streams, check if tracks are ready
          )
            session.started('remote', response);
          else
            // Firefox requires waiting until the stream event fired
            session.rtcMediaHandler.onAddStream = function() {
              session.started('remote', response);
              session.rtcMediaHandler.onAddStream = undefined;
            }
        },
        /*
         * onFailure
...
Message.prototype.send = function(target, body, options) {
  ...
  this.request = new JsSIP.OutgoingRequest(JsSIP.C.MESSAGE, target, this.ua, requestParams, extraHeaders);
...
// Horrible Plugin requirement, must be declared globally
function plugin0() {
...
if('chrome' in window) {
    ...
    WebRTC.getScreenId = function(callback) {

        // Make sure that external script is loaded
        if (!iframe.isLoaded) {
            setTimeout(function(){
              JsSIP.WebRTC.getScreenId(callback);
            }, 500);
            return;
        }
...
var plug = false;
if(document.body && 'WebSocket' in window) {
  if('ActiveXObject' in window) {
    try {
      plug = new ActiveXObject('Tem.TemWebRTCPlugin');
    } catch(e) {}
  } else if(!(window.RTCPeerConnection || window.webkitRTCPeerConnection || window.mozRTCPeerConnection) && navigator.plugins) {
    var idx = navigator.plugins.length;
    while(idx--) {
      if(navigator.plugins[idx].name=='TemWebRTCPlugin') {
        plug = true;
        break;
      }
    }
