...
{
    $libXmlEntityLoaderEnabled = libxml_disable_entity_loader(true);
    $xml_text = preg_replace('/<!DOCTYPE(?:[^\\[]+\\[[^\\]]+\\])+?[^>]*>/i', '', $xml_text);
    $xml_text = preg_replace('/&+(?!(?:amp)|(?:gt)|(?:lt))([^;]+?);/i', '', $xml_text);
  ...
  libxml_disable_entity_loader($libXmlEntityLoaderEnabled);
  $stackpos=0;
  $stack=array();
