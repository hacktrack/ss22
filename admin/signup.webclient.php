...
class Signup extends SignupAncestor
{
	...
	public function __construct()
	{
		...
		if(isset($_GET['auth_id']) || isset($_GET['conference']) || isset($_GET['legacy'])) return;
		$this->permanentLogin();
		$this->ssoLogin();		
	...
	protected function getTemplateData()
	{
        ...
        if($this->include_webmail){
		     $this->templateData['domain'] = Tools::getHostDomain();
        }else{
			 $this->templateData['lang']['login_screen']['title'] = '...';
		}
	...
	protected function getTemplate()
	{
		...
		if ($this->videoBox) {
			...
			if (isset($_REQUEST['atoken'])) {
				...
				try{
					...
					$this->cookie->setcookie('PHPSESSID_LOGIN',null,-1,'/');
                    			if($_GET['remember']){
                       				$this->cookie->setcookie('permanentLogin','i='.$sid,mktime(0,0,0,1,1,2030),'/');
                    			}else{
                        			$this->cookie->setcookie('permanentLogin','',mktime(0,0,0,1,1,2030),'/');
                    			}
					header("Location: ./".self::getClearGet());
					die();
				}catch(Exc $e){
					$location = './?eid='.$e->wmcode;
					if($_REQUEST['notifyuri']){
						$location.='&notifyuri='.urlencode($_REQUEST['notifyuri']);
					}
			...
			if (isset($_COOKIE['lastLogin'])) {
				...
				if($this->isAvailableLanguage($parse[0])) {
					$lastLanguage = $parse[0];
				}
			}
			if (isset($_GET['language']) && $this->isAvailableLanguage($_GET['language'])) {
				$lastLanguage = $_GET['language'];
			}
		...
		} elseif ($_REQUEST['interface']=='basic' || $_REQUEST['interface']=='pda') {
			...
			if (isset($this->eDataInfo)){
				if (isset($this->eDataInfo['form']) && is_array($this->eDataInfo['form'])){
					foreach($this->eDataInfo['form'] as $key=>$val){
						if (is_array($val)){
							$this->eDataInfo['form'][$key]=slToolsPHP::htmlspecialchars_array($val);
						}else{
	...
	{		
        ...
        $this->api = IceWarpAPI::instance('api');
		if(!$this->include_webmail){
          return;    
        }
		...
		if(!isset($this->settings['layout']['title'])){
			$this->settings['layout']['title'] = $this->language_data['login_screen']['title'];
		}
    ...
    private function isAvailableLanguage($code)
    {
        $languages=array("en","ar","bg","cs","dk","de","el","es","la","fa","fr","hr","is","it","kr","lv","hu","jp","nl","no","pl","pt","ru","sk","fi","se","th","tr","si");
	...
	public function getClearGet($qs=array())
	{
		...
		if (isset($_GET['method'])) {
			$qs[]='method='.$_GET['method'];
		}	
		if (isset($_GET['RoomID'])) {
			$qs[]='RoomID='.$_GET['RoomID'];
		}	
		if (isset($_GET['PostID'])) {
			$qs[]='PostID='.$_GET['PostID'];
		}	
		if (isset($_GET['roomid'])) {
			$qs[]='roomid='.$_GET['roomid'];
		}	
    ...
    protected function defines()
    {
       ...
       define( 'WORKING_MODEL_PATH' , '../basic/' );
        
       chdir( MODEL_PATH );
       $browserInfo = $this->browser_info();
       $this->include_webmail =  ($this->videoBox || $this->meeting || ($this->directLogin && isset($_REQUEST['atoken']) ) || ($this->pda && !$this->desktop)) || ($browserInfo['browser'] == 'msie' && $browserInfo['version'] <= 8);

       if($this->include_webmail){
           	require_once(WORKING_MODEL_PATH.'model/webmail.php' );
       }else{
       		require_once(WORKING_MODEL_PATH.'../server/inc/require.php' );
       }
    ...
    private function init()
    {
		...
		$this->OSName = $OS['name'];
		$this->OSCode = $OS['code'];
		$this->pda = $mdr['pda'] || $_REQUEST['interface']=='basic' || $_REQUEST['interface']=='pda';		
        ...
        $this->videoBox = isset($_GET['videoBox']);
        
		          
        if(isset($_GET['pda'])){$this->pda=true;}
        $this->unsupportedIE = ($this->browser_info()['browser'] == 'msie' && $this->browser_info()['version'] <= 8);
    }
}
?>
