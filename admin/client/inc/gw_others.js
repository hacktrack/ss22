...
//********************************************************************************
_me.load = function(aResources,sDataSet,sDataPath){
	storage.library('wm_storage');
	sDataSet = sDataSet || 'storage';

	dataSet.add(sDataSet,sDataPath,WMStorage.get({'resources':aResources}),true);
...
_me.getItem = function(sResourceName, sItemName, sStorage){
...
_me.setItem = function(sResourceName, sItemName, v){
	var tmp = {};
		tmp[sItemName] = v;
	var rsc = GWOthers.set(sResourceName, tmp, 'storage');
};
...
_me.getDefaultValues = function(sResourceName)
{
	...
	switch(sResourceName){
        case 'GLOBAL_SETTINGS':
			aValues = {'HTTP_PORT':sPrimaryAccountHTTP};
			break;
		...
		case 'IM':
			aValues = {'ENTER_SEND':1,'AUTO_CHAT':1,'AUTO_STATUS':1,'SMILES':1,'STATUS_DND':1};
			break;

		case 'MAIL_SETTINGS_DEFAULT':
			aValues = {'REPLY_MYSELF':0,'CHECK_SUBJECT':1,'SIGN_TOP':1,'FONT_FAMILY':0,'FONT_SIZE':0,'TEXT_DIRECTION':'LTR','SPELLCHECKER':'en','HTML_MESSAGE':0,'REPLY_MESSAGE':0,'READ_CONFIRMATION':0,'SAVE_SENT_MESSAGE':1,'ENCRYPT':0,'SIGN':0,'REPLY_TO_ADDRESS':'','PRIORITY':3,'CHARSET':'UTF-8'};
			break;

		case 'MAIL_SETTINGS_GENERAL':
			aValues = {'SOUND_NOTIFY':0,'AUTOUPDATE':1,'AUTOUPDATE_MINUTES':5,'MOVE_TO_TRASH':1,'FORWARD_MESSAGES':'inline','AUTOSAVE':1,'AUTOSAVE_MINUTES':5,'DEFAULT_FLAG':'1','AUTO_RECIPIENT_TO_ADDRESSBOOK':'0','AUTO_SHOW_IMAGES':'0','AUTOCLEAR_TRASH_DAYS':'30'};
			break;

		case 'LAYOUT_SETTINGS':
			aValues = {'INTERFACES':'abp','ACTIVITY':0,'SSL_SWFATT':0,'LOGIN_STYLE':'blue','LOGIN_CLIENT':'advanced','SKIN':'default','SKIN_STYLE':'default','LANGUAGE':'en','DISABLE_SWFUPLOAD':0,'DISABLE_OFFICE':0,'LOGIN_LOGO':'login_logo.png','DATE_FORMAT':0,'TIME_FORMAT':0,'INIT_PAGE':'i','CONFIRM_EXIT':1,'NOTIFICATIONS':0,'BANNER_ENABLED':1};
			break;
		...
		case 'CALENDAR_SETTINGS':
			aValues = {'WEEK_BEGINS':'sunday','BEGIN_ON_TODAY':0,'DAY_BEGINS':8,'DAY_ENDS':16,'TEMPERATURE':'C','AUTOCLEAR_TRASH_DAYS':'30'};
			break;
		...
		case 'RESTRICTIONS':
			aValues = {'SORTSTRING':0,'DISABLE_OTHERACCOUNTS':0,'DISABLE_CHANGEPASS':0,'DISABLE_SIGNUP':1,'DISABLE_DROPBOX':0,'GAL_SUGGEST':1};
			break;

		case 'SIP':
			aValues = {'MODE':'integrate'};
	}
