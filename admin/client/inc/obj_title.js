...
_me._add = function(sTitle,iTime,bForce){
	if (iTime){
		...
		this.__buffer.push([sTitle, id, iTime, setTimeout('try{'+this._pathName+'._remove('+ id +','+ (bForce?'true':'false') +');}catch(r){}', iTime*1000)]);
		return id;
	}
	else{
		...
		for(var i = 1; i<this.__buffer.length; i++)
			if (this.__buffer[i][1] == id){
				...
				if (bForce || gui.__focus)
					...
					this.__buffer[i][3] = setTimeout('try{'+this._pathName+'._remove('+ id +');}catch(r){}', (this.__buffer[i][2])*1000);
