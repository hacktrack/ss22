...
class slWidgetTree extends slWidget
{
	...
	{	
		...
		{				
			...
			{	
				$node['_VALUES_']['ueid']=urlencode($node['_VALUES_']['id']);		
				...
				{							
					...
					if ($node['_VALUES_']['id']==urldecode($active))
					{
			...
			if (trim($val)!='')
			{
				...
				elseif ((count($prev)==2 && count($node)==1) || $last) {$nodeClass.=' end';}
				else {$nodeClass.='';}
